...
class SAE():
    ...
    def evaluate(self):
        ...
        if args.reduced == 'no':
            with open(f'confusion_matrix/synt_{args.dataset}_method_{args.method}_n_att_{args.att}_cm_F2S.pickle', 'wb') as handle:
                pickle.dump(cm_F2S, handle, protocol=pickle.HIGHEST_PROTOCOL)
        elif args.reduced == 'yes':
            with open(f'confusion_matrix/reduced_synt_{args.dataset}_method_{args.method}_n_att_{args.att}_sub_{args.num_subset}_cm_F2S.pickle', 'wb') as handle:
                pickle.dump(cm_F2S, handle, protocol=pickle.HIGHEST_PROTOCOL)

        if args.reduced == 'no':
            with open(f'confusion_matrix/synt_{args.dataset}_method_{args.method}_n_att_{args.att}_cm_S2F.pickle', 'wb') as handle:
                pickle.dump(cm_S2F, handle, protocol=pickle.HIGHEST_PROTOCOL)
        elif args.reduced == 'yes':
            with open(f'confusion_matrix/reduced_synt_{args.dataset}_method_{args.method}_n_att_{args.att}_sub_{args.num_subset}_cm_S2F.pickle', 'wb') as handle:
                pickle.dump(cm_S2F, handle, protocol=pickle.HIGHEST_PROTOCOL)
