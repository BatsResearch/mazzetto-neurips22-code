...
#           
        ...
        acc, preds = compute_per_class_acc_gzsl(test_label, predicted_label, target_classes, in_package)
        return acc, preds
...
#           
            ...
            start = end
        acc_gzsl, _ = compute_per_class_acc_gzsl(test_label, predicted_label_gzsl, unseen_classes, in_package)
        acc_zs, preds_return = compute_per_class_acc_gzsl(test_label, predicted_label_zsl, unseen_classes, in_package)
        ...
        return acc_gzsl,acc_zs_t, preds_return
...
#        pdb.set_trace()
    return per_class_accuracies.mean().item(), (test_label, predicted_label)

def eval_zs_gzsl(dataloader,model,device,bias_seen,bias_unseen, method):
    model.eval()
    ...
    with torch.no_grad():
        ...
        acc_seen, _ = val_gzsl(test_seen_feature, test_seen_label, seenclasses, in_package,bias=bias_seen)
        if method == 'test':
            acc_novel,acc_zs, preds = val_zs_gzsl(test_unseen_feature, test_unseen_label, unseenclasses, in_package,bias = bias_unseen)
        elif method == 'trainval':
            acc_novel,acc_zs, preds = val_zs_gzsl(test_seen_feature, test_seen_label, seenclasses, in_package,bias = bias_unseen)
    ...
    return acc_seen, acc_novel, H, acc_zs, (preds, test_unseen_label)
