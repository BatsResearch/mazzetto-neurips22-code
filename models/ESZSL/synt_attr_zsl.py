...
class ESZSL():
	...
	def __init__(self):
		...
		res101 = pickle.load(open(res, "rb"))
	...
	def evaluate(self, alpha, gamma):
		...
		if args.reduced == 'no':
			with open(f'confusion_matrix/synt_{args.dataset}_method_{args.method}_n_att_{args.att}_cm.pickle', 'wb') as handle:
				pickle.dump(cm_save, handle, protocol=pickle.HIGHEST_PROTOCOL)
		elif args.reduced == 'yes':
			with open(f'confusion_matrix/reduced_synt_{args.dataset}_method_{args.method}_n_att_{args.att}_sub_{args.num_subset}_cm.pickle', 'wb') as handle:
				pickle.dump(cm_save, handle, protocol=pickle.HIGHEST_PROTOCOL)
